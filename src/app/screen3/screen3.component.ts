...
export class Screen3Component implements OnInit {
  ...
  constructor(private httpClient:HttpClient) {     
    this.httpClient.get("https://my-json-server.typicode.com/AmalBoussaada/jsonfakerdirectory/questions")
    .subscribe((data:any[])=>{
      console.log(data);
      if (data.length){
    ...
    this.httpClient.get("https://my-json-server.typicode.com/AmalBoussaada/jsonfakerdirectory/question4Options")
    .subscribe((data:any[])=>{
      console.log(data);
      if (data.length){
        for (let index = 0; index < data.length; index++) {
